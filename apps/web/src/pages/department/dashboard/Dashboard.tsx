...
export default function Dashboard() {
  ...
  const tasks = useMemo(() => {
    return (allTasks || []).filter(
      (t) => isAdmin || t.departmentId === myDepartmentId
    )
  }, [allTasks, isAdmin, myDepartmentId])
  ...
  const metrics = [
    {
      ...
      desc: isAdmin ? "Across all departments" : "In your department",
      icon: IconDashboard,
          ...
          <header className="flex items-end justify-between">
            <div className="space-y-1">
              ...
              <p className="text-base text-zinc-500 dark:text-zinc-400">
                {isAdmin ? "System Overview" : (currentUserObj?.departments?.[0]?.name ?? "Department Overview")} ·{" "}
